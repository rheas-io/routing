...
const uriValidator_1 = require("./validators/uriValidator");
const hostValidator_1 = require("./validators/hostValidator");
const notFound_1 = require("@rheas/errors/notFound");
const methodValidator_1 = require("./validators/methodValidator");
const schemeValidator_1 = require("./validators/schemeValidator");
const methoNotAllowed_1 = require("@rheas/errors/methoNotAllowed");
class Router extends route_1.Route {
    /**
    ...
    constructor(app) {
         ...
         * @var array
         */
        this.middlewares_list = {};
        /**
         * Cache of route by names.
         *
         * @var object
         */
        this._namedEndpoints = {};
        /**
         * Cache of routes grouped by methods.
         *
         * @var object
         */
        this._methodEndpoints = {};
        /**
    ...
    handleError(err, req, res) {
        const exceptionHandler = this.app.get('error');
        if (exceptionHandler) {
     ...
     * @param res
     */
    async dispatchToRoute(route, req, res) {
        const destination = this === route ? this.routeHandler.bind(this) : this.resolveDestination(route, req);
        return await new requestPipeline_1.RequestPipeline()
     ...
     * @param response
     */
    async routeHandler(request, response) {
     ...
     * @param route
     */
    middlewarePipesOfRoute(route) {
        return route.middlewaresToResolve().reduce((prev, current) => {
     ...
     * @param middleware
     */
    routeRequiresMiddleware(route, middleware) {
     ...
     * @param middleware
     */
    middlewareNameParams(middleware) {
        let [name, ...others] = middleware.trim().split(':');
        let params = others.join(':');
     ...
     * @param req
     */
    resolveDestination(route, request) {
        return async (req, res) => {
            let controllerAction = route.getAction();
            if (typeof controllerAction !== 'function') {
                controllerAction = this.resolveController(controllerAction);
            }
            const params = request.params();
            return await controllerAction(req, res, ...params);
        };
    }
    /**
     ...
     * @param filename
     */
    controllerScript(filename) {
        const rootPath = this.app.get('path.root') || '';
        const controllerDir = support_1.Str.path(this.controllerPath);
        const controllerFile = support_1.Str.path(filename);
     ...
     */
    otherMethods(request, original_method) {
        const _methods = [];
     ...
     * @param request
     */
    matchAgainstRoutes(routes, request) {
     ...
     * @param validators
     */
    routeMatches(route, request, validators) {
